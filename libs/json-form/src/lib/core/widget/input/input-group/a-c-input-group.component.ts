...
import { GroupSearchComponent } from './dialog-group-search/group-search.component';

@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'ac-input-group',
  templateUrl: './a-c-input-group.component.html'
})
// eslint-disable-next-line @angular-eslint/no-conflicting-lifecycle
export class ACInputGroupComponent extends AppBaseInputModel<AppInputBaseConfig> implements DoCheck, OnInit, OnChanges {

  @Input() suffixIcon       : string;
  @Input() groupType       : string  = "group";
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    const control = this.findCurrentFormControl(appFormConfig, this.control);
    const object = appFormConfig.formGroup.value;
  ...
  ngDoCheck(): void {
    ...
    if (change) {
      change.forEachChangedItem(item => {
        ...
        if (item.key === '_value' && item.currentValue !== null) {
          const appFormConfig: AppFormConfig = this.appFormConfig;
          const control = this.findCurrentFormControl(appFormConfig, this.control);
          const object = appFormConfig.formGroup.value;
  ...
  onSuffixIconClicked() {
    const groupDialogModel: AppDialogModel = {
      title: this.groupType==="userGroup"? "گروه های کاربری":".",
      dialogData: {groupType: this.groupType==="userGroup"? "1":"0"},
      size: DialogSizeEnum.md,
      onClose: (res: Group) => {
        if (res) {
          if(this.value !== res.id) {
            this.value = res.id
    ...
    this.appDialogService.openDialog<GroupSearchComponent, Group>(GroupSearchComponent, groupDialogModel)
  }
