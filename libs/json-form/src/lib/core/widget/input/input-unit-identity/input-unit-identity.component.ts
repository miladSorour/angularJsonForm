...
import { UnitIdentitySearchComponent } from './dialog-unit-identity-search/unit-identity-search.component';

@Component({
  // eslint-disable-next-line @angular-eslint/component-selector
  selector: 'app-input-unit-identity',
  templateUrl: 'input-unit-identity.component.html',
})
// eslint-disable-next-line @angular-eslint/no-conflicting-lifecycle
export class AppInputUnitIdentityComponent extends AppBaseInputModel<AppInputBaseConfig> implements DoCheck, OnInit, OnChanges {

  @Input() suffixIcon       : string;
  @Input() suffixIconClicked: any;
  @Output() valueChange     = new EventEmitter();
  @Input() selectedTitleInput:string| undefined = '';
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    const selectedTitleCol = appFormConfig.formControl.find((control: AppFormControlModel) => control.name === this.control.name).selectedTitle;
    appFormConfig.formGroup.valueChanges.subscribe(value => {
      if (value[selectedTitleCol])
        this.selectedTitle = value[selectedTitleCol]
    })
  ...
  onSuffixIconClicked() {
    const groupDialogModel: AppDialogModel = {
      ...
      onClose: (res: UnitIdentity) => {
        if (res) {
          this.value = res.id
          this.inputForm.patchValue({myInput: res.id})
          this.selectedTitle = res.title
        }
      }
    }
    this.appDialogService.openDialog<UnitIdentitySearchComponent, UnitIdentity>(UnitIdentitySearchComponent, groupDialogModel)
  }
